...
type LoadFn = Mutex<Box<dyn Fn() -> Result<LlamaModel, Error> + Send>>;

struct PoolState {
    models: Vec<Arc<LlamaModel>>,
    used: usize,
    max: usize,
    loading: bool,
}
...
impl ModelPool {
    pub fn acquire(&self) -> Result<PoolPermit<'_>, Error> {
        loop {
...
pub(crate) fn get_or_load_model(
    interpreter_key: &str,
    num_copies: usize,
    load_fn: impl Fn() -> Result<LlamaModel, Error> + Send + 'static,
) -> Result<(Arc<LlamaModel>, Arc<ModelPool>), Error> {
    ...
    let first_model = Arc::new(load_fn()?);
