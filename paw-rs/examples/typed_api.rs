//! Static typing with base model sharing via `PawFn<T>`.
//!
//! Uses `PawFn::<Qwen3_0_6B>` so the compiler knows the interpreter at
//! compile time.  Multiple `PawFn<T>` instances of the same `T` share
//! a single base model in memory (keyed by `TypeId::of::<T>()`).
//!
//! Usage:
